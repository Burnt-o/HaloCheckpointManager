...
namespace HCMExternal.Services.Hotkeys.Impl
{
    ...
    public class Hotkey : IHotkey
    {
        ...
        // null represents gamepad unbound

        public XInputButton? GamepadButton { get; private set; } = null;

        public void UpdateBindings((VirtualKey, Guid)? vk, XInputButton? gp)
        {
            VirtualKeyCode = vk;
            GamepadButton = gp;
        ...
        {
            if (VirtualKeyCode == null && GamepadButton == null)
                return "Unbound";
            else
                return (VirtualKeyCode?.Item1.ToString() ?? "") + 
                    ((VirtualKeyCode != null && GamepadButton != null) ? ", " : "") + 
