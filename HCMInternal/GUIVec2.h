...
template< bool shouldRenderVertically, int decimalPrecision>
...
public:
	...
	GUIVec2(GameState implGame, ToolTipCollection tooltip, std::string labelText, std::shared_ptr<Setting<SimpleMath::Vector2>> optionVec2)
		: IGUIElement(implGame, std::nullopt, tooltip), mLabelText(labelText), mOptionVec2Weak(optionVec2)
	{
		...
		this->currentHeight = shouldRenderVertically ? GUIFrameHeightWithSpacing * 3 : GUIFrameHeightWithSpacing;
	...
	void render(HotkeyRenderer& hotkeyRenderer) override
	{
		...
		if constexpr (shouldRenderVertically)
		{
			...
			if (ImGui::InputFloat(std::format("x: ##{}", mLabelText).c_str(), &mOptionVec2->GetValueDisplay().x, 0, 0, decimalPrecisionFormatter.c_str()))
			{
			...
			ImGui::SetNextItemWidth(100);
			if (ImGui::InputFloat(std::format("y: ##{}", mLabelText).c_str(), &mOptionVec2->GetValueDisplay().y, 0, 0, decimalPrecisionFormatter.c_str()))
			{
				PLOG_VERBOSE << "GUIVec2 (" << getName() << ") firing toggle event, new value: " << mOptionVec2->GetValueDisplay();
				auto& newThread = mUpdateSettingThreads.emplace_back(std::thread([optionToggle = mOptionVec2]() { optionToggle->UpdateValueWithInput(); }));
				newThread.detach();
