...
public:
	...
	GUIColourPicker(GameState implGame, ToolTipCollection tooltip, std::string labelText, std::shared_ptr<BinarySetting<SimpleMath::Vector4>> optionVec4)
	...
	void render(HotkeyRenderer& hotkeyRenderer) override
	{
		...
		if (ImGui::ColorEdit3(mLabelText.c_str(), &mOptionVec4->GetValueDisplay().x, editFlags))
		{
			PLOG_VERBOSE << "GUIColourPicker (" << getName() << ") firing toggle event, new value: " << mOptionVec4->GetValueDisplay();
