...
class GUIVec3CustomLabels : public IGUIElement {
...
public:
	...
	GUIVec3CustomLabels(GameState implGame, ToolTipCollection tooltip, std::string labelText, std::shared_ptr<Setting<SimpleMath::Vector3>> optionVec3, std::string xLabel, std::string yLabel, std::string zLabel)
		: IGUIElement(implGame, std::nullopt, tooltip), mLabelText(labelText), mOptionVec3Weak(optionVec3), mxLabel(xLabel), myLabel(yLabel), mzLabel(zLabel)
	{
		...
		PLOG_VERBOSE << "Constructing GUIVec3CustomLabels, name: " << getName();
