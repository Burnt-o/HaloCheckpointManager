...
class DumpCore::DumpCoreImpl
{
private:
	GameState mGame;

	// event callbacks
	eventpp::ScopedRemover<ActionEvent> mDumpCoreEventCallback;
	// injected services
	std::shared_ptr<IGetMCCVersion> getMCCVer;
	std::shared_ptr<GameStateHook> gameStateHook;
	std::shared_ptr<RPCClientInternal> rpcClient;
	...
	void onDump()
	{
		...
		try
		{
			...
			auto currentSaveFolder = rpcClient.get()->getDumpInfo();
			...
			std::string versionString = getMCCVer.get()->getMCCVersionAsString().data();
...
public:
	DumpCoreImpl(GameState game, IDIContainer dicon) : mGame(game),
		getMCCVer(dicon.Resolve<IGetMCCVersion>()), gameStateHook(dicon.Resolve<GameStateHook>()), rpcClient(dicon.Resolve<RPCClientInternal>())
	{
		mDumpCoreEventCallback.append(OptionsState::dumpCoreEvent.get()->append([this]() { onDump(); }));
	}
};



DumpCore::DumpCore(GameState game, IDIContainer dicon) : impl(std::make_unique<DumpCoreImpl>(game, dicon)) {}
DumpCore::~DumpCore() = default;
