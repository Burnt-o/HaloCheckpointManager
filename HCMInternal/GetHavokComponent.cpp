...
private:
	...
	//data
	std::shared_ptr<MultilevelPointer> havokComponentMetaDataTable;
	uintptr_t havokComponentMetaDataTable_cached = 0;

	int64_t havokComponentHeaderStride;
...
public:
		...
		MCCStateChangedCallback(dicon.Resolve<IMCCStateHook>().lock()->getMCCStateChangedEvent(), [this](const MCCState& state) { onGameStateChange(state); })
	{
		...
		havokComponentMetaDataTable = ptr->getData<std::shared_ptr<MultilevelPointer>>(nameof(havokComponentMetaDataTable), game);

		havokComponentHeaderStride = *ptr->getData<std::shared_ptr<int64_t>>(nameof(havokComponentHeaderStride), game).get();
	...
	virtual uintptr_t getHavokComponent(Datum havokDatum) override
	{
		...
		if (!havokComponentMetaDataTable_cached)
		{
			...
			if (!havokComponentMetaDataTable->resolve(&havokComponentMetaDataTable_cached)) throw HCMRuntimeException(std::format("Failed to resolve havokComponentMetaDataTable: {}", MultilevelPointer::GetLastError()));
		}

		auto havokComponentHeaderAddress = havokComponentMetaDataTable_cached + (havokComponentHeaderStride * havokDatum.index);
