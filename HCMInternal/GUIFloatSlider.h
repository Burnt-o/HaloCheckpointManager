...
template<float minVal, float maxVal>
class GUIFloatSlider : public IGUIElement {
...
public:
	...
	GUIFloatSlider(GameState implGame, ToolTipCollection tooltip, std::string labelText, std::shared_ptr<BinarySetting<float>> optionFloat)
		: IGUIElement(implGame, std::nullopt, tooltip), mLabelText(labelText), mOptionFloatWeak(optionFloat)
	{
		...
		PLOG_VERBOSE << "Constructing GUIFloatSlider, name: " << getName();
		PLOG_DEBUG << "GUIFloatSlider.getOptionName: " << std::hex << mOptionFloatWeak.lock()->getOptionName();
	...
	void render(HotkeyRenderer& hotkeyRenderer) override
	{
		...
		if (ImGui::SliderFloat(mLabelText.c_str(), &mOptionFloat->GetValueDisplay(), minVal, maxVal))
		{
			PLOG_VERBOSE << "GUIFloatSlider (" << getName() << ") firing toggle event, new value: " << mOptionFloat->GetValueDisplay();
