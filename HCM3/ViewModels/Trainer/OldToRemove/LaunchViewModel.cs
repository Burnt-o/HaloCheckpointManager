...
namespace HCM3.ViewModels
{
    public class LaunchViewModel : ActionToggleBase
    {
        ...
        public LaunchViewModel(string effectText, CheatManagerService? cheatManagerService, HotkeyManager? hotkeyManager) 
        {
        ...
        public void ExecuteLaunch()
        {
            try
            {
                //todo: update speed settings property first
                this.CheatManagerService.ListOfCheats["ForceLaunch"].DoCheat();
            }
            catch (Exception ex)
            {
