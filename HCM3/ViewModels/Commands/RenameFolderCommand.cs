...
namespace HCM3.ViewModels.Commands
{
    ...
    {
        internal RenameFolderCommand(CheckpointViewModel checkpointViewModel, CheckpointServices checkpointServices)
        {
        ...
        public void Execute(object? parameter)
        {
            try
            {
                ...
                CheckpointServices.RenameFolder(CheckpointViewModel.SelectedSaveFolder);
                CheckpointViewModel.RefreshSaveFolderTree();
            }
            catch (Exception ex)
            {
                System.Windows.MessageBox.Show("Failed to rename folder! \n" + ex.ToString(), "HaloCheckpointManager Error", System.Windows.MessageBoxButton.OK);
            }
