...
{
    public class ForceCoreLoadCommand : ITrainerCommand
    {
        internal ForceCoreLoadCommand(TrainerViewModel trainerViewModel, TrainerServices trainerServices)
            {
        ...
        public void Execute(object? parameter)
        {
            try
            {
                TrainerServices.ForceCoreLoad(TrainerViewModel.SelectedGame);

            }
            catch (Exception ex)
            {
                System.Windows.MessageBox.Show("Failed to Force Core Load! \n" + ex.Message, "HaloCheckpointManager Error", System.Windows.MessageBoxButton.OK);
            }
