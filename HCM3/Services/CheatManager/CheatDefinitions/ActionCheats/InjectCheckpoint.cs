...
{
    internal class InjectCheckpoint : AbstractActionCheat
    {

        // Reads checkpoint data from a file on disk and injects it into the current-checkpoint-slot of in-game-memory
        public InjectCheckpoint(ServicesGroup servicesGroup) : base(servicesGroup)
        {
        ...
        {
            Checkpoint? selectedCheckpoint = CheckpointViewModel.SelectedCheckpoint;
            SaveFolder? selectedSaveFolder = CheckpointViewModel.SelectedSaveFolder;
            ...
            if (haloState.GameState == GameStateEnum.Halo1 && Properties.Settings.Default.H1Cores)
            {
                ...
                // Let the user know
                this.CheatManagerService.SendMessage("Core save injected.", haloState);
                return;
            }
            ...
            Dictionary<string, bool> injectRequirements = (Dictionary<string, bool>)this.DataPointersService.GetRequiredPointers($"{haloState.GameState.To2Letters()}_InjectRequirements", haloState);
            ...
            foreach (KeyValuePair<string, bool> kvp in injectRequirements)
            {
                if (kvp.Value)
                {
                    requiredPointerNames.Add($"{haloState.GameState.To2Letters()}_" + kvp.Key);
                }
            }
            ...
            if (Properties.Settings.Default.WarnInjectWrongLevel)
            {
                ...
                try
                {
                    ...
                    int? levelNameOffset = (int?)this.DataPointersService.GetRequiredPointers($"{haloState.GameState.To2Letters()}_CheckpointData_LevelCode", haloState);

                    if (levelNameOffset == null) throw new Exception("levelNameOffset was null!");
                    if (haloState.LevelInfo.LevelCode == null) throw new Exception("CurrentLevelCode was null!");
                    ...
                    levelString = levelString.Substring(0, haloState.LevelInfo.LevelCode.Length);
                    Trace.WriteLine("length of currentlevelcode: " + haloState.LevelInfo.LevelCode.Length + ", length of new substring: " + levelString.Length);
                ...
                if (levelString != null && haloState.LevelInfo.LevelCode != null && levelString.Substring(0, 3) != haloState.LevelInfo.LevelCode.Substring(0, 3))
                {
                    string message = "It appears the checkpoint you're injecting is not on the same level as the current in-game level. This is very likely to cause the game to crash. Proceed anyway?\nCheckpoint level: " + levelString + "\nCurrent level: " + haloState.LevelInfo.LevelCode;
                    Trace.WriteLine(message);
                    Trace.WriteLine("current level ?: " + haloState.LevelInfo.LevelCode);
            ...
            if (Properties.Settings.Default.WarnInjectWrongVersion)
            {
                ...
                if (versionString != null && versionString != haloState.MCCVersion)
                {
            ...
            if (haloState.IsMultiplayer() == true && CheckpointDataIsNull(inGameCheckpointLocation))
            {
                try
                {
                    Trace.WriteLine("Forcing a checkpoint to un-nullify the checkpoint data");
                    ReadWrite.Pointer checkpoint = (ReadWrite.Pointer)this.DataPointersService.GetRequiredPointers($"{haloState.GameState.To2Letters()}_ForceCheckpoint", haloState);
            ...
            this.CheatManagerService.SendMessage("Checkpoint injected.", haloState);
        ...
        private bool CheckpointDataIsNull(ReadWrite.Pointer cplocation)
        {
