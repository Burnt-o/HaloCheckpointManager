    ...
    public partial class DataPointersService
    {
        ...
        public object ParseObject(XElement entry, string type)
        {
            ...
            switch (type)
            {
                ...
                case "ulong":
                    returnObject = ParseHexNumberUlong(entry.Element("Offset")?.Value);
                    break;

                case "long":
                    returnObject = ParseHexNumberLong(entry.Element("Offset")?.Value);
                    break;
        ...
        long ParseHexNumberLong(string? s)
        {
            if (s == null)
                throw new Exception("int was null");
            ...
            return s.StartsWith("0x") ? Convert.ToInt64(s.Substring(2), 16) * sign : Convert.ToInt64(s) * sign;
        }
        ...
        ulong ParseHexNumberUlong(string? s)
        {
            if (s == null)
                throw new Exception("uint was null");
            ...
            return s.StartsWith("0x") ? Convert.ToUInt64(s.Substring(2), 16) : Convert.ToUInt64(s);
        }
        ...
        DetourInfoObject ParseDetourInfoObject(XElement entry)
        {
            ...
            if (entry.Element("SymbolPointers") != null && entry.Element("SymbolPointers").HasElements)
            {
                foreach (XElement Pointer in entry.Element("SymbolPointers").Elements())
                {
                    ...
                    else if (Pointer.Name == "InternalFunction")
                    { 
                    string? symbolName = Pointer.Value ?? throw new Exception("failed reading symbol for symbolPointer (internal)");
                        SymbolPointers.Add(symbolName, symbolName);
                    }
